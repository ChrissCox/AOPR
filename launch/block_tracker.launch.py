...
def generate_launch_description():
    ...
    params_file_dec = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(get_package_share_directory('block_detection'),
                                   'config',
                                   'block_tracker_params_example.yaml'),
        description='Full path to params file for ball_tracker nodes.')
    ...
    tune_detection_dec = DeclareLaunchArgument(
        'tune_detection',
        default_value='false',
        description='Enables tuning mode for detection')
